...
const isFailedMetric = (metrics) => {
    if (!metrics || typeof metrics !== 'object') return false;
    return metrics.error === "fetch failed" && metrics.score === null && 
           metrics.audits && Object.keys(metrics.audits).length === 0;
};
...
export async function GET(_req, { params }) {
    ...
    try {
        ...
        // 4. SEO & PERFORMANCE - MODIFIED LOGIC HERE
        const shouldRunSeo = mainHref && (!company.seoMetrics || isFailedMetric(company.seoMetrics));

        if (shouldRunSeo) {
            console.log(`SEO metrics missing or failed. Running for ID ${id}...`);
            const seoData = await getSeo(mainHref, id);
            updates.seoMetrics = seoData;
            company.seoMetrics = seoData;
        }

        const shouldRunPerformance = mainHref && (!company.performanceMetrics || isFailedMetric(company.performanceMetrics));

        if (shouldRunPerformance) {
            console.log(`Performance metrics missing or failed. Running for ID ${id}...`);
